    ...
    Public Class ManageController
        ...
        Public Async Function Index(message As System.Nullable(Of ManageMessageId)) As Task(Of ActionResult)
            ...
            Dim model = New IndexViewModel() With {
                ...
                .IsGoogleAuthenticatorEnabled = usr.IsGoogleAuthenticatorEnabled,
                .IsAdmin = UserManager.IsInRole(usr.Id, "Admin")
           }
            Return View(model)
        End Function
        ...
        Public Async Function DisableGoogleAuthenticator() As Task(Of ActionResult)

            Dim Usr As ApplicationUser = Await UserManager.FindByIdAsync(User.Identity.GetUserId())
            If Usr IsNot Nothing Then

                Usr.IsGoogleAuthenticatorEnabled = False
                Usr.GoogleAuthenticatorSecretKey = Nothing
                Await UserManager.UpdateAsync(Usr)

                Await SignInManager.SignInAsync(Usr, False, False)
            End If
            Return RedirectToAction("Index", "Manage")
        End Function

        <HttpGet>
        Public Async Function EnableGoogleAuthenticator() As Task(Of ActionResult)

            Dim Usr As ApplicationUser = Await UserManager.FindByIdAsync(User.Identity.GetUserId())
            Dim secretKey As String = GoogleAuthProvider.GenerateSecretKey
            ...
            Dim model As New GoogleAuthenticatorViewModel With
        {
           .SetupCode = tfa.GenerateSetupCode("VBJay Licensing", userName, secretKey, 300, 300),
           .SecretKey = secretKey,
           .IsGoogleAuthenticationEnabled = Usr.IsGoogleAuthenticatorEnabled
        }
        ...
        <HttpPost>
        <ValidateAntiForgeryToken>
        Public Async Function EnableGoogleAuthenticator(model As GoogleAuthenticatorViewModel) As Task(Of ActionResult)

            Dim tfa As New Google.Authenticator.TwoFactorAuthenticator
            If tfa.ValidateTwoFactorPIN(model.SecretKey, model.Code, TimeSpan.FromMinutes(1.5)) Then
                Dim usr As ApplicationUser = Await UserManager.FindByIdAsync(User.Identity.GetUserId())

                usr.IsGoogleAuthenticatorEnabled = True
                usr.GoogleAuthenticatorSecretKey = model.SecretKey
                usr.TwoFactorEnabled = True
                'todo encrypt authentication secret key
                Await UserManager.UpdateAsync(usr)
                Return RedirectToAction("Index", "Manage")
            Else
            ...
            model.SetupCode = tfa.GenerateSetupCode("VBJay Licensing", User.Identity.GetUserName(), model.SecretKey, 300, 300)
            Return View(model)
        End Function
        <HttpGet>
        Public Async Function AddToAdmin(Model As AddAdminViewModel) As Task(Of ActionResult)
            If ModelState.IsValid Then
                'todo remove this in production
                Dim usr As ApplicationUser = Await UserManager.FindByIdAsync(Model.UserID)
                If usr Is Nothing Then
                    ModelState.AddModelError("UserID", "User not found.")
                Else
                    Await UserManager.AddToRoleAsync(usr.Id, "Admin")
